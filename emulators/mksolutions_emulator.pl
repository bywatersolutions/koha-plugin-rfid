...
get '/mkStaffStationAPI/getItems' => sub ($c) {
    ...
    my @items = map { 
        { 
            barcode => $_,
            security => ${$config->{barcodes}->{$_}->{security}} ? 'true' : 'false'
        }
        ...
        foreach my $item (@items) {
            $sub_xml .= qq{
                <item>
                    ...
                    <tags>
                        <tag>
                            <tag_id>Not applicable</tag_id>
                            <part_num>1</part_num>
                        </tag>
                    </tags>
                    <is_secure>$item->{security}</is_secure>
...
<success>false</success>
<exceptiondetail>
    <message>No RFID tag found with this barcode.</message>
</exceptiondetail>
</rfid>
        };
    }
...
@@ index.html.ep
% layout 'default';
% title 'MK Solutions RFID Emulator';

<h3>MK Solutions RFID Emulator</h3>
